...
import { AiOutlineUpload, AiOutlineReload, AiOutlineDownload, AiOutlineDelete, AiOutlineMenu } from 'react-icons/ai'
...
// This function correctly embeds a 300 DPI tag into the PNG data.
...
const LogoUploader: FC<LogoUploaderProps> = ({ logo, onFileSelect, onDelete, isProcessing, index }) => {
    ...
    const uploaderBaseClasses = `relative flex-1 min-w-[120px] aspect-square flex flex-col items-center justify-center gap-2 bg-gray-900/70 border-2 border-dashed p-2 rounded-lg transition-all duration-300 group`;
    const uploaderDragClasses = isDraggedOver ? 'border-blue-400 scale-105 bg-blue-500/10' : 'border-gray-600';
    return (
        <motion.div
                ...
                {logo.src ? (
                    ...
                    <motion.label key="uploader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} htmlFor={`logo-upload-${logo.id}`} className="w-full h-full flex flex-col items-center justify-center text-center cursor-pointer text-gray-400">
                        ...
                        {error ?
                            ...
                            <span className="text-xs text-gray-500 mt-1">{isDraggedOver ? 'Release to upload' : 'Click or Drag & Drop'}</span>
                        }
...
export default function PosterWithMultipleLogosEditor() {
    ...
    // Modified: Default export format is now 'jpeg'
    ...
    const handleFileSelect = useCallback((id: string, file: File) => {
        ...
        startProgressAnimation(() => {
            ...
            img.onerror = () => {
                ...
                const errorMessage = 'Failed to load image. Please try again.';
    ...
    const executeExport = async () => {
        ...
        // This condition checks for 'Use Original' resolution.
        ...
        // This line embeds the 300 DPI tag for PNG files. This is not supported for JPEGs.
    ...
    return (
        <div className="flex min-h-screen bg-transparent text-gray-300 font-sans">
            {/* Note: Sidebar component is not provided, so this will cause an error unless you have it in your project */}
            ...
            <main className={`flex-1 flex flex-col p-6 lg:p-8 transition-all duration-300 ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-0'}`}>
                ...
                <button onClick={toggleSidebar} className="fixed top-4 left-4 z-50 lg:hidden p-2 rounded-full bg-gray-800 text-white shadow-lg">
                ...
                <div className="flex-1 flex flex-col lg:flex-row lg:items-center lg:justify-center gap-6">
                    {/* Left Panel - Logo Uploads */}
                    <motion.div 
                        initial={{ opacity: 0, x: -50 }} 
                        animate={{ opacity: 1, x: 0 }} 
                        transition={{ duration: 0.5 }}
                        className="w-full lg:w-1/4 bg-gray-800/50 border border-gray-700 rounded-xl p-6 flex flex-col gap-6"
                    >
                        <div>
                    ...
                    <div className="flex-1 w-full relative min-h-[50vh] flex flex-col items-center justify-center">
                    ...
                    {/* Right Panel - Adjustments and Export */}
                    <motion.div 
                        initial={{ opacity: 0, x: 50 }} 
                        animate={{ opacity: 1, x: 0 }} 
                        transition={{ duration: 0.5 }}
                ...
                <AnimatePresence>
                    {showExportModal && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
                            {/* Modified: The export modal now uses the same styling as the card from the single poster editor. */}
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-md p-6 bg-white/5 rounded-3xl shadow-2xl backdrop-blur-lg border-4 border-white/20 hover:border-white/40 transition-all duration-300">
                                <h2 className="text-2xl font-bold text-gray-200 mb-6">Export Settings</h2>
                                <div className="space-y-4">
                                    <div>
